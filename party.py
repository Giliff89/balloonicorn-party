...
def is_mel(name, email):
    ...
    >>> is_mel("mel", "g@gmail.com")
    True

    >>> is_mel("yo", "mel@ubermelon.com")
    True

    """
    polymelism = ["mel", "mel melitpolski",
                  "melitpolski", "mel@ubermelon.com"]

    return name.lower() in polymelism or email.lower() in polymelism


def most_and_least_common_type(treats):
    """Given list of treats, return {most, least} common types.

    >>> most_and_least_common_type([{'type': 'dessert'}, {'type': 'drink'}, {'type': 'appetizer'}, {'type': 'dessert'}])
    ('dessert', 'drink')
